...
You can run Pylint on the command line to generate a report on the quality of the codebase and save it to a file named "pylint-results.txt":

```commandline
pylint hypernetx --reports=y --output=pylint-results.txt
```

To configure Pylint, one option is to add the configuration in setup.cfg. For example,
the following configuration will disable checking for invalid names.
