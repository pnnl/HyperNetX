...
class StaticEntity(namedtuple):
    ...
    **Honor System**
    ...
    def __init__(self, uid=None, elements=[], entity=None, **props):
        super().__init__()

        self._uid = uid or '_'

        if entity is not None:
            if isinstance(entity, EntitySet):
                ...
                pass
            elif is instance(entity, Entity):
                if entity.is_bipartite:
                    self._elements = entity.elements
                    self._memberships = entity.memberships
                    self.__dict__.update(entity.properties)
                else:
                    raise HyperNetXError('StaticEntitySet cannot be formed from entity when entity.is_bipartite == False')
            else:
                raise TypeError('entity object must be of type Entity, EntitySet or StaticEntitySet')
...
class StaticEntitySet(StaticEntity):
    ...
    *Entities that are elements of the same EntitySet, may not contain each other as elements.*
    ...
    def __init__(self, uid, elements=[], **props):
        ...
        # bipartite condition should automatically be met
