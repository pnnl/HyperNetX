...
def precompute_modularity_parameters(HG):
...
def HNX_2section(HG):
    ...
    for e in HG.edges:
        ...
        try:
            ...
            w = 1 / (len(E) - 1)
        s.extend([(k[0], k[1], w) for k in itertools.combinations(E.elements, 2)])  # BP
...
def HNX_Kumar(HG, delta=.01):
    ...
    W = {e: HG.edges[e].weight for e in HG.edges}
    # build graph
    G = HNX_2section(HG)
    # apply clustering
    ...
    while diff > delta:
        ...
        for i in HG.edges:
            e = HG.edges[i]
            reweight = sum([1 / (1 + HG.size(i, c)) for c in CH]) * (HG.size(i) + len(CH)) / HG.number_of_edges()
...
def DeltaEC(HG, P, v, a, b, wdc):
    ...
    if HG.isstatic:
        memberships = HG.nodes.memberships[v]
    else:
        memberships = HG.nodes[v].memberships
