...
class Entity:
    ...
    Parameters
    ----------
    entity : pandas.DataFrame, dict of lists or sets, list of lists or sets, Entity, optional
    ...
    def __init__(
        self,
        entity: Optional[
            Entity | pd.DataFrame | dict[T, [Iterable[T]]] | Iterable[Iterable[T]] | np.ndarray ] = None,  ###bp relationships cell properties for df - how best to annotate a dict of node : dict objects?
        # data: Optional[np.ndarray] = None, ### add this as a choice of entity
        column_names: Optional[Sequence[T]] = [], ### for entity object will create or change column names of dataframe 
        data_cols:Optional[Sequence[T] | int] = 0, ### columns with categorical data to be compared, int = # of columns from left, if 0 all columns will be used
        uid: Optional[Hashable] = None, 
        weights: Optional[Sequence[int | float] | [int | float]] = None, ## list of values or single value to assign to all  ## only used for dataframe, numpy array entities
        ## list of weights only used if entity is a pandas dataframe or a numpy ndarray
        weight_col: Optional[str | int] = None,  ### reference to entity = dataframe or ndarray column or dictionary property (in which case it must have the property on all edge,node pairs)
        ## weight col is ignored if weights are given. The weight_col will be called "cell_weights"
        aggregateby: Optional[str | Map[str|int,str]] = None,

        # #### dfp properties
        properties: Optional[pd.DataFrame | dict[T, dict[T, dict[Any, Any]]]] = None,  ## must map the column name and value to a set of properties
        ...
        if isinstance(entity, Entity): 
            self = deepcopy(entity) ### this should change to get_state when ready
            self._uid = uid
            self._state_dict = {}
        else:
            ...
            if isinstance(entity, (pd.DataFrame,np.ndarray)):
                self._dataframe = pd.DataFrame(deepcopy(entity))
                if len(column_names) == len(self._dataframe.columns):
                    d = dict(list(zip(entity.columns,column_names)))
                    self._dataframe = deepcopy(entity).rename(columns=d)
                elif len(column_names) > 0:
                    warnings.warn('The column_names could not be used, possibly the number did not match the number of columns in entity.')
                if data_cols == 0:
                    self._data_cols = self._dataframe.columns
                elif isinstance(data_cols, int):
                    self._data_cols = self._dataframe.columns[:data_cols]
                elif set(data_cols).issubset(self._dataframe.columns):
                    self._data_cols = data_cols
                else:
                    raise HyperNetXError('Specified data_cols were not usable. must be non-negative integer or subset of the column_names.')
            ...
            # used to fill the first level/column
                ...
                self._dataframe = pd.DataFrame(
                    {'e': entity.index.to_list(), 'n': entity.values}
                )
                if len(column_names)==2:
                    d = dict(list(zip([0,1],column_names)))
                    self._dataframe = self._dataframe.rename(columns = d)
                elif len(column_names) != 0:
                    warnings.warn('The column_names could not be used, exactly 2 column names are required when using a dictionary or list of lists. Entity will be generated with default column names.')
                self._data_cols = self._dataframe.columns

            # create an empty Entity
            else: 
                self._dataframe = pd.DataFrame(columns=column_names)
                self._data_cols = self._dataframe.columns
            ...
            for c in self._dataframe.columns:
                self._labels[c] = self._dataframe[c].drop_duplicates().values
            self._state_dict["labels"] = self._labels
            ...
            #     if len(column_names) != self._dimsize:
            #         warnings.warn(f"Entity dataframe has {self._dimsize} columns but only {len(column_names)} column names were given. The column_names keyword values will not be used.")
            #     else:
            #         cols = self._dataframe.column_names
            #         renamecols = {cols[cdx]:column_names[cdx] for cdx in range(self._dimsize) }
            #         self._dataframe.rename(columns=renamecols)

            # if data_cols is not None:
            #     if isinstance(data_cols, int):
            #         self._data_cols = self._dataframe.columns[:data_cols]
            #     else:
            #         self._data_cols = [d for d in data_cols if d in self._dataframe.
            #         columns]
            # else:
            #     self._data_cols = self._dataframe.columns
            ...
            self._dataframe, self._cell_weight_col = assign_weights(
                self._dataframe, weights=weights or weight_col)
