...
class Hypergraph:
    ...
    A hypergraph allows for multi-edges by distinguishing edges by their identifiers
    instead of their contents. For example,     let V = {1,2,3} and E = {e1,e2,e3},  
    ...
    2. From a dictionary of iterables (elements must be hashable): ::
    ...
    5. From a pandas.DataFrame. The dataframe must have at least two columns
         with headers and there can be no nans. By default the first column corresponds to the edge names and the
         second column to the node names. You can specify the columns by restricting the dataframe to the
    ...
    Parameters
    ----------
    setsystem : (optional) Entity, EntitySet, dict, iterable,
    ...
    def __init__(
        ...
        aggregateby="sum",
        use_nwhy=False,
        filepath=None,  ###bp: should probably remove this???
        **kwargs,
    ):
