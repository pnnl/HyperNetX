...
from hypernetx.algorithms.matching_algorithms import greedy_d_approximation
# from hypernetx.algorithms.matching_algorithms import hedcs_based_approximation
...
#     assert len(result) == 2
#     assert all(edge in ['e1', 'e2'] for edge in result)
#
#     hypergraph = Hypergraph({'e1': {1, 2, 3}, 'e2': {4, 5, 6}, 'e3': {7, 8, 9}, 'e4': {1, 4, 7}, 'e5': {2, 5, 8}, 'e6': {3, 6, 9}})
#     result = hedcs_based_approximation(hypergraph, 3, 3)
#     assert len(result) == 3
#     assert all(edge in ['e1', 'e2', 'e3'] for edge in result)
#
#     hypergraph = Hypergraph({'e1': {1, 2, 3}, 'e2': {2, 3, 4}, 'e3': {3, 4, 5}, 'e4': {5, 6, 7}, 'e5': {6, 7, 8}, 'e6': {7, 8, 9}})
#     result = hedcs_based_approximation(hypergraph, 3, 2)
#     assert len(result) == 2
#     assert all(edge in ['e1', 'e4'] for edge in result)
#
#     # Test large input
#     large_hypergraph = Hypergraph({f'e{i}': {i, i+1, i+2} for i in range(1, 100, 3)})
#     result = hedcs_based_approximation(large_hypergraph, 3, 10)
#     assert len(result) == 33
#     assert all(edge in [f'e{i}' for i in range(1, 100, 3)] for edge in result)
#
